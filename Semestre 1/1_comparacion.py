class Recurciones(object):
	def __init__(self):
		pass
	#Recurcion de pila
	def simple_pila(self,num,n):
		if isinstance(num,int) and isinstance(n,int):
			return self.simple_pile_aux(num,n-1)

	def simple_pila_aux(self,num,n):
		if n==0:
			return 0
		else: retrun (num*n) + self.simple_pila_aux(num, n-1)
	...
	def simple_cola_aux(self,num,n,resultado):
		if n==0:
			return resultado
		else: return self.simple_cola_aus(num, n-1, resultado+num*n)
